...
async function updateProfile(req, res) {
  try {
    const { nom, prenoms, tel, date_naissance, email, mdp } = req.body;
    ...
    if (prenoms) { fields.push(`prenoms = $${i++}`); values.push(prenoms); }
    if (tel !== undefined) { fields.push(`tel = $${i++}`); values.push(tel || null); }
    if (date_naissance !== undefined) { fields.push(`date_naissance = $${i++}`); values.push(date_naissance || null); }
    if (email) {
