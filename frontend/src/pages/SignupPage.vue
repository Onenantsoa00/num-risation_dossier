<template>
  <div class="auth-bg">
    <div class="auth-panel" style="max-width: 520px">
      <div class="brand-mark q-mb-xs">ORDSEC <span>Dossiers</span></div>
      <p class="text-grey-7 q-mb-lg">Créer un compte</p>

      <q-form @submit.prevent="onSubmit" class="q-gutter-md">
        <div class="row q-col-gutter-md">
          <div class="col-6">
            <q-input v-model="form.nom" label="Nom" outlined dense :rules="[v => !!v || 'Requis']" />
          </div>
          <div class="col-6">
            <q-input v-model="form.prenoms" label="Prénoms" outlined dense :rules="[v => !!v || 'Requis']" />
          </div>
        </div>
        <q-input v-model="form.email" type="email" label="Email" outlined dense :rules="[v => !!v || 'Requis']" />
        ...
        <q-banner v-if="error" class="bg-red-1 text-negative" rounded dense>{{ error }}</q-banner>
        <q-btn type="submit" color="primary" label="Créer le compte" class="full-width" :loading="loading" unelevated />
      </q-form>

      <div class="q-mt-md text-center">
        Déjà inscrit ?
        <router-link :to="{ name: 'login' }" class="text-primary text-weight-medium">Se connecter</router-link>
      </div>
    </div>
  </div>
</template>
