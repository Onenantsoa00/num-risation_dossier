    ...
    <q-header elevated class="bg-primary text-white">
      <q-toolbar>
        ...
        <q-toolbar-title class="text-weight-bold">
          ORDSEC <span class="text-accent">Dossiers</span>
        </q-toolbar-title>

        <q-btn flat round dense icon="notifications" :to="{ name: 'notifications' }">
          <q-badge v-if="notif.unread" color="accent" floating>{{ notif.unread }}</q-badge>
        </q-btn>

        <q-btn flat dense no-caps class="q-ml-sm" :to="{ name: 'profil' }">
          <q-avatar size="28px" color="secondary" text-color="white" class="q-mr-sm">
            {{ initials }}
          </q-avatar>
    ...
    <q-drawer v-model="leftDrawer" show-if-above bordered :width="250">
      <q-list padding>
        ...
        <q-item clickable v-ripple :to="{ name: 'notifications' }">
          ...
          <q-item-section>
            Notifications
            <q-badge v-if="notif.unread" color="negative" class="q-ml-sm">{{ notif.unread }}</q-badge>
          </q-item-section>
        </q-item>
        ...
        <q-item>
          <q-item-section>
            ...
            <q-item-label class="text-weight-medium">{{ auth.role }}</q-item-label>
          </q-item-section>
        </q-item>
